...
namespace DAES.BLL
{
    public class Custom
    {
        ...
        private readonly GestionProcesosContext gpContext = new GestionProcesosContext();
        ...
        public byte[] CrearDocumentoConfOficio(DocOficio docofi, Organizacion org, bool tieneTabla)
        {
            ...
            var vacio2 = new Paragraph("soy vacio", _fontBlanco);
            cellA2.AddElement(vacio2);
            ...
            using (SistemaIntegradoContext context = new SistemaIntegradoContext())
            {
                try
                {
                    ...
                    if (response.IsValid)
                    {
                        ...
                        //Objeto para el nuevo PDF firmado
                        var nuevoregistro = new DocOficio();
                        //si el documento ya tiene folio no solicitarlo nuevamente                        if (string.IsNullOrWhiteSpace(documento.Folio))
                        {
                            ...
                            try
                            {
                                ...
                                //Para crear el PDF despues de Firma Electronica.

                                nuevoregistro.WorkFlowId = documento.WorkFlowId;
                                nuevoregistro.ProcesoId = documento.ProcesoId;
                                nuevoregistro.NUMERO_REGISTRO = documento.NUMERO_REGISTRO;
                                nuevoregistro.ANTECEDENTES = documento.ANTECEDENTES;
                                nuevoregistro.MATERIA = documento.MATERIA;
                                nuevoregistro.DE_DOC = documento.DE_DOC;
                                nuevoregistro.A_DOC = documento.A_DOC;
                                nuevoregistro.DIRECCION = documento.DIRECCION;
                                nuevoregistro.CORREO = documento.CORREO;
                                nuevoregistro.Firmado = false;
                                nuevoregistro.Tabla = documento.Tabla;
                                nuevoregistro.Parrafo1 = documento.Parrafo1;
                                nuevoregistro.Parrafo2 = documento.Parrafo2;
                                nuevoregistro.AUTORES = documento.AUTORES;
                                nuevoregistro.Folio = documento.Folio;
                                nuevoregistro.TieneDirectorio = documento.TieneDirectorio;
                                nuevoregistro.FileName = "DocumentoCreadoTask_" + documento.WorkFlowId + "_" + string.Format("{0:dd/MM/yyyy}", DateTime.Now) + ".pdf";
                        ...
                        var docto = hsms.SignOficio(documento.File, idsFirma, documento.WorkFlowId, documento.Folio, url_tramites_en_linea.Valor, qr, documento, proc);
                        nuevoregistro.Content = docto;
                        //documento.Signed = true;
                        nuevoregistro.Firmado = true;
                        nuevoregistro.DocOficioId = documento.DocOficioId + 1;
                        nuevoregistro.WorkFlowId = documento.WorkFlowId + 1;
                        ...
                        db.Documento.Add(new Documento()
                        {
                            ...
                            Content = nuevoregistro.Content,
                            TipoDocumentoId = (int)Infrastructure.Enum.TipoDocumento.Oficio,
                            Firmado = true,
                            //Signed = false,
                            WorkflowId = nuevoregistro.WorkFlowId,
            ...
            using (SistemaIntegradoContext context = new SistemaIntegradoContext())
            {
                ...
                if (TipoDocumentoId == (int)Infrastructure.Enum.TipoDocumento.Articulo8voTransitorio)
                {
                    ...
                    if (transitorio != null)
                    {
                        ...
                        parrafo_uno = parrafo_uno.Replace("[OBJETO]", transitorio != null && transitorio.Objeto != null ? "#" + transitorio.Objeto.ToString() + "#" : "[ERROR_Transitorio]");
                    }
                    else
                    {
                    ...
                    var obj = parafo1.Count();
                    foreach (string element in parafo1)
                    {
                        if (index % 2 == 0 || obj == index)
                        {
                    ...
                    // PARRAFO 2 - FIN
                    // PARRAFO 3 - INI
                    if (organizacion.ReformaAnteriors.Any() || organizacion.ReformaPosteriors.Any())
                    {
                ...
                if (TipoDocumentoId == (int)Infrastructure.Enum.TipoDocumento.Estatutos)
                {
                        ...
                        if (organizacion.ExistenciaAnteriors.Any()) // estatuto anterior 2003
                        {
                                ...
                                if (ReformaUlti.IdReformaPost != 0) // cuenta con al menos una reforma sin obs o con obs aprobada
                                {
                                    ...
                                    if (ObsReforma != null)
                                    {
                                        if (ObsReforma.AprobacionId == (int)Infrastructure.Enum.Aprobacion.aprobada)
                                        {
                                            ...
                                            parrafo2[0] = parrafo2[0].Replace("[NUMEROOFICIO]", ReformaUlti.ObservacionReformas.FirstOrDefault().NumeroOficio != null ? "Dicha acta estatutaria se encuentra aprobada por oficio ordinario N° #" + organizacion.ObservacionReformas.FirstOrDefault().NumeroOficio + "#" : ReformaUlti.ObservacionReformas.FirstOrDefault().FechaOficio != null ? "" : ".");
                                            parrafo2[0] = parrafo2[0].Replace("[FECHAOFICIO]", ReformaUlti.ObservacionReformas.FirstOrDefault().FechaOficio != null && ReformaUlti.ObservacionReformas.FirstOrDefault().FechaOficio != null ? "de fecha #" + string.Format("{0:dd} de {0:MMMM} del {0:yyyy}", ReformaUlti.ObservacionReformas.FirstOrDefault().FechaOficio.Value) + "#." : "");
                            ...
                            if (organizacion.ReformaAnteriors.Any() && ReformaVigente == false) // estatuto anterior 2003  reforma anterior
                            {
                                ...
                                if (ReformaUlti.IdReformaAnterior != 0) //cuenta con al menos una reforma sin obs o con obs aprobada
                                {
                                    ...
                                    parrafo2[0] = parrafo2[0].Replace("[TIPONORMA]", ReformaUlti.TipoNormaId != null ? "Dicha acta estatutaria se encuentra #aprobada# por #" + ReformaUlti.TipoNorma.Nombre + "#" : "ERROR_Reforma");
                                    parrafo2[0] = parrafo2[0].Replace("[NUMERONORMA]", ReformaUlti.NNorma != null ? "N° #" + ReformaUlti.NNorma + "#" : "ERROR_Reforma");
                                    parrafo2[0] = parrafo2[0].Replace("[FECHANORMA]", ReformaUlti.FechaNorma.HasValue ? "de fecha #" + string.Format("{0:dd} de {0:MMMM} del {0:yyyy}", ReformaUlti.FechaNorma.Value) + "#" : "ERROR_Reforma");
                                    parrafo2[0] = parrafo2[0].Replace("[AUTORIZADOPOR]", ReformaUlti.AutorizadoPor != null ? "del #" + ReformaUlti.AutorizadoPor + "#." : "ERROR_Reforma");
                                    parrafo2[0] = parrafo2[0].Replace("[NUMEROOFICIO]", "");
                                    parrafo2[0] = parrafo2[0].Replace("[FECHAOFICIO]", "");
                                    if (parrafo2[0].Contains("ERROR_Reforma"))
                                    {
                            ...
                            if (ReformaVigente == false) // NO TIENE REFORMAS anterior 2003
                            {
                                ...
                                if (ObsLegal != null)
                                {
                                    ...
                                    {
                                        parrafo2[0] = parrafo2[0].Replace("[TIPONORMA]", legalAnt.TipoNormaId != null ? "Dicha acta estatutaria se encuentra #aprobada# por #" + legalAnt.tipoNorma.Nombre + "#" : "ERROR_Opcional");
                            ...
                            if (organizacion.ReformaAnteriors.Any()) // estatuto anterior 2003 reforma ante 
                            {
                                ...
                                foreach (var Re in Reforma)
                                {
                                    ...
                                    IndReforma++;
                                    var ObsReformaAnt = organizacion.ObservacionReformas.FirstOrDefault(q => q.IdReformaAnterior == Re.IdReformaAnterior);
                        ...
                        else if (organizacion.ExistenciaPosteriors.FirstOrDefault(q => q.OrganizacionId == organizacion.OrganizacionId) != null)
                        {
                            ...
                            if (ReformaUlti.IdReformaPost != 0) //cuenta con al menos una reforma sin obs o con obs aprobada
                            {
                                ...
                                if (ReformaUlti.ObservacionReformas.Any())
                                {
                                    ...
                                    parrafo2[0] = parrafo2[0].Replace("[FECHANORMA]", "");
                                    parrafo2[0] = parrafo2[0].Replace("[AUTORIZADOPOR]", "");
                                    parrafo2[0] = parrafo2[0].Replace("[NUMEROOFICIO]", ReformaUlti.ObservacionReformas.FirstOrDefault().NumeroOficio != null ? "Dicha acta estatutaria se encuentra aprobada por oficio ordinario N° #" + ReformaUlti.ObservacionReformas.FirstOrDefault().NumeroOficio + "#" : ReformaUlti.ObservacionReformas.FirstOrDefault().FechaOficio != null ? "" : ".");
                                    parrafo2[0] = parrafo2[0].Replace("[FECHAOFICIO]", ReformaUlti.ObservacionReformas.FirstOrDefault().FechaOficio != null && ReformaUlti.ObservacionReformas.FirstOrDefault().FechaOficio != null ? "de fecha #" + string.Format("{0:dd} de {0:MMMM} del {0:yyyy}", ReformaUlti.ObservacionReformas.FirstOrDefault().FechaOficio.Value) + "#." : "");
                    ...
                    else //estatuto gremial / CONSUMIDORES
                    {
                        ...
                        if (legalGremial.ObservacionLegals.Any() && legalGremial.AprobacionId != (int)Infrastructure.Enum.Aprobacion.aprobada)
                        {
                            parrafo_dos = parrafo_dos.Replace("[NUMEROOFICIO]", legalGremial.ObservacionLegals.FirstOrDefault().NumeroOficio != null ? "Dicha acta estatutaria se encuentra " + legalGremial.Aprobacion.Nombre + " por oficio ordinario N° #" + legalGremial.ObservacionLegals.FirstOrDefault().NumeroOficio + "#" : organizacion.ObservacionLegals.FirstOrDefault().FechaOficio != null ? "" : ".");
                        ...
                        {
                            parrafo_dos = parrafo_dos.Replace("[NUMEROOFICIO]", legalGremial.NumeroOficio != null ? "Dicha acta estatutaria se encuentra " + legalGremial.Aprobacion.Nombre + " por oficio ordinario N° #" + legalGremial.NumeroOficio + "#" : legalGremial.FechaOficio != null ? "" : ".");
                            parrafo_dos = parrafo_dos.Replace("[FECHAOFICIO]", legalGremial.FechaOficio != null && legalGremial.FechaOficio != null ? "de fecha #" + string.Format("{0:dd} de {0:MMMM} del {0:yyyy}", legalGremial.FechaOficio.Value) + "#." : "");
                        }
                        ...
                        if (organizacion.ReformaAGACs.Any())
                        {
                            ...
                            foreach (var Re in ReformaGremial)
                            {
                                ...
                                if (IndReforma == 4 && Re.IdReformaAGAC != ReformaGremial.LastOrDefault().IdReformaAGAC)
                                {
                                    doc.NewPage();
                                }
                                else if ((IndReforma - 4) % (6 + (cantiObsReforma / 2)) == 0 && Re.IdReformaAGAC != ReformaGremial.LastOrDefault().IdReformaAGAC)
                                {
                                    doc.NewPage();
                                }
                                //PARRAFO 4 Reforma - FIN
                            ...
                            if (comision.Any())
                            {
                                ...
                                if (aux.FechaComiLiquiJuntaSocios != null)
                                {
                                    foreach (var item in comision.ToList())
                                    {
                                        ...
                                        {
                                            var cargo = context.Cargo.FirstOrDefault(q => q.CargoId == item.CargoId);
                                            if (item.NombreCompleto != null && cargo.Nombre != null && aux.FechaInicio.HasValue == true)
                                            {
                                                filaCompleta = true;
                                                break;
                                            }
                                ...
                                if (filaCompleta == true)
                                {
                                    foreach (var item in comision.ToList())
                                    {
                                        if (item.EsMiembro)
                                        {
                                            ...
                                            PdfPCell cellini = new PdfPCell(new Phrase(aux.FechaInicio.HasValue ? string.Format("{0:dd-MM-yyyy}", aux.FechaInicio.Value) : string.Empty, _fontStandard));
                                            cellini.HorizontalAlignment = Element.ALIGN_CENTER;
                                            table.AddCell(cellini);
                                            PdfPCell cellfin = new PdfPCell(new Phrase(aux.FechaTermino.HasValue ? string.Format("{0:dd-MM-yyyy}", aux.FechaTermino.Value) : string.Empty, _fontStandard));
                                            cellfin.HorizontalAlignment = Element.ALIGN_CENTER;
                                            table.AddCell(cellfin);
        ...
        public Proceso ProcesoStart(Proceso obj)
        {
            using (SistemaIntegradoContext context = new SistemaIntegradoContext())
            {
                ...
                if (proceso.DefinicionProceso.DefinicionProcesoId == (int)Infrastructure.Enum.DefinicionProceso.SolicitudCertificadoManual ||
                    proceso.DefinicionProceso.DefinicionProcesoId == (int)Infrastructure.Enum.DefinicionProceso.SolicitudCertificadoAutomatico)
                {
                    obj.ProcesoGD = true;
                }
                proceso.ProcesoGD = obj.ProcesoGD;
                ...
                // en el caso de que el proceso se origine a partir de un documento de Gestor Antiguo, asociar datos
                if (obj.ProcesoGD)
                {
                ...
                {
                    if (obj.ProcesoGPId > 0)
                    {
                        var documento = gpContext.DocumentoGP.First(q => q.ProcesoId == obj.ProcesoGPId);
                        if (documento != null)
                        {
                            proceso.ProcesoGPId = documento.ProcesoId;
                            proceso.Correlativo = documento.ProcesoId.ToString();
                        }
                    }
                }
                ...
                if (definicionworkflow != null)
                {
                    ...
                    {
                        var adjuntos = gpContext.DocumentoGP.Where(q => q.ProcesoId == proceso.ProcesoGPId).ToList();
                ...
                if (proceso.ProcesoGPId > 0)
                {
                    ...
                    var workflowGP = workflowGPList.First(q => !q.Terminada && !q.Anulada);

                    ProcesoGP.EstadoProcesoId = (int)Infrastructure.Enum.EstadoProcesoGP.Terminado;
                    workflowGP.Terminada = true;
                    workflowGP.TipoAprobacionId = (int)Infrastructure.Enum.TipoAprobacionGP.Aprobado;
                }
                //guardar proceso
                ...
                try
                {
                    ...
                    if (response.IsValid)
                    {
                        ...
                        if (string.IsNullOrWhiteSpace(documento.Folio))
                        {
                            ...
                            try
                            {
                                var folios = folio.GetFolio(string.Join(", ", rubrica.Email), TipoDocto, "ECONOMIA" /*persona.SubSecretaria*/);
