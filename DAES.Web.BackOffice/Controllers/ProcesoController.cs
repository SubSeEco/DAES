...
namespace DAES.Web.BackOffice.Controllers
{
    ...
    public class ProcesoController : Controller
    {
        ...
        private readonly GestionProcesosContext _context = new GestionProcesosContext();
        ...
        public ActionResult DocumentoGP()
        {
            ...
            var modelWork = _context.WorkflowGP.Where(q => !q.Terminada && !q.Anulada && q.DefinicionWorkflowId == DefWork && q.Pl_UndCod == 200900)
                .Select(p => p.ProcesoId)
                .SelectMany(procesoId => _context.GDGP.Where(g => g.DestinoUnidadCodigo == "200900" && g.ProcesoGP.EstadoProcesoGP.EstadoProcesoId == 1
                && g.ProcesoId == procesoId)
                .Select(q => new DTODocumentoGP()
                {
                    Id = q.GDId,
                    Doc_Correlativo = q.ProcesoId.ToString(),
                    Doc_FechaCreacion = q.FechaIngreso.Value,
                    ReferenciaGD = q.Referencia,
                    ObservacionGD = q.Observacion
                })).ToList();
        ...
        public ActionResult DocumentoCreateGP(int? id)
        {
            ViewBag.DefinicionProcesoId = new SelectList(db.DefinicionProceso.Where(q => q.Habilitado).OrderBy(q => q.Nombre), "DefinicionProcesoId", "Nombre");
            ViewBag.UserId = new SelectList(db.Users.Where(q => q.Habilitado).OrderBy(q => q.UserName).ToList(), "Id", "UserName");
            var gd = _context.GDGP.Find(id);

            var model = new DTOProceso()
            {
                GDGP = gd,
                ProcesoGP = _context.ProcesoGP.First(q => q.ProcesoId == gd.ProcesoId),
                WorkflowGP = _context.WorkflowGP.Where(q => q.ProcesoId == gd.ProcesoId).OrderByDescending(q => q.WorkflowId).ToList(),
                DocumentoGP = _context.DocumentoGP.Where(q => q.ProcesoId == gd.ProcesoId && q.Activo).ToList()
            };
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult DocumentoCreateGP(Model.DTO.DTOProceso model)
        {
            if (model.DefinicionProcesoId != (int)Infrastructure.Enum.DefinicionProceso.ConstitucionOP)
            {
                if (!db.Organizacion.Any(q => q.OrganizacionId == model.OrganizacionId))
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    var proceso = new Proceso()
                    {
                        ...
                        Observacion = model.Observacion,
                        UserId = model.UserId,
                        ProcesoGPId = model.GDGP.ProcesoId,
                        ProcesoGD = false
                    };
